...
| 日常工具 | 音乐播放器 👑   | 读取本地音乐并播放，支持封面、歌词和歌单            |
| 日常工具 | 黄历 👑      | 展示黄历相关信息                        |
| 日常工具 | 图片轮播 👑    | 图片轮播展示组件                        |
| 日常工具 | 赛博木鱼 👑    | 轻量娱乐和放松组件                       |
...
音乐播放器用于读取和管理电脑中的本地音乐，支持在主页中直接播放，也可以离开主页后通过悬浮迷你播放器继续控制。它不会上传音乐文件，所有音乐都来自你选择的本地文件夹。

目前支持：

- **本地音乐文件夹**：选择电脑中的音乐目录即可播放，支持 MP3、WAV、OGG、FLAC、AAC、M4A 等格式；
- **子目录扫描**：可以开启递归扫描多级音乐目录；
- **音乐元数据**：读取歌名、歌手、专辑、时长等信息，并建立本地索引，避免每次重复读取大量标签；
- **封面与歌词**：优先使用音频文件内嵌数据，也支持读取同目录下的同名图片和 `.lrc` 歌词文件；
- **播放列表**：支持收藏音乐、创建歌单、导入导出 M3U8 歌单和 JSON 备份；
- **悬浮迷你播放器**：离开主页后仍可控制播放、暂停和切换音乐。

使用限制：音乐播放器是会员组件，并且依赖思源桌面端的本地文件系统，网页端、Docker 和移动端无法读取电脑音乐文件夹；同一设备、同一主页界面只应放置一个音乐播放器。
