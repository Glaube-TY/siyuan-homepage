...
export const readCandidateDocsOutputSchema = z.object({
  ...
  duplicateReadItemCount: z.number().int().min(0).optional(),
  truncatedDocCount: z.number().int().min(0).optional(),
  totalReturnedChars: z.number().int().min(0).optional(),
  largestDocChars: z.number().int().min(0).optional(),
