...
const HANDLE_QUOTED_PATTERN = /handle\s+["'`]([^"'`]+)["'`]/gi;
const HANDLE_BARE_PATTERN = /\bhandle\s+([A-Za-z0-9_.\-+:]+)/gi;

const KEYED_FIELD_PATTERN =
  /\b(docId|blockId|notebookId|path|realDocId|realBlockId|realPath|evidenceDocId|evidenceBlockId)\s*[=:]\s*([^\s,;]+)/gi;
 ...
 *   / 原始 handle 的字符串，也会先被兜底替换为 `[redacted]` 再截断。
 ...
 * - `handle "xxx"` / `handle xxx` 形式 → `handle [redacted]`
 * - `docId/blockId/notebookId/path/realDocId/realBlockId/realPath/evidenceDocId/evidenceBlockId = xxx`
 *   字段值 → `[redacted]`
 *
...
export function scrubString(input: string): string {
  ...
  out = out.replace(HANDLE_QUOTED_PATTERN, () => `handle ${REDACTED}`);
  out = out.replace(HANDLE_BARE_PATTERN, () => `handle ${REDACTED}`);
