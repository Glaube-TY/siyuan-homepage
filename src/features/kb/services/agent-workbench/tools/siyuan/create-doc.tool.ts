...
export interface CreateDocDeps {
  executeCreateDoc(args: CreateDocInput): Promise<{ output: CreateDocOutput }>;
}
...
export function createCreateDocTool(deps: CreateDocDeps): ToolContract<CreateDocInput, CreateDocOutput> {
  return {
    ...
    async execute(_ctx: ToolRuntimeContext, args: CreateDocInput): Promise<ToolResult<CreateDocOutput>> {
      try {
        const result = await deps.executeCreateDoc(args);
