...
export function createSiyuanAssetManageTool(deps: SiyuanAssetManageDeps) {
  return createGenericSiyuanTool({
    ...
    title: "管理资源",
    description: "重命名资源、管理思源 PDF .sya 标注、设置 OCR、执行 OCR、删除未使用资源或重建资源内容索引。set_annotation 传结构化 annotation 对象可设置标注，传 clear:true 可清除全部标注；remove_unused_one/batch 只能删除本轮 disposable 测试资源；full_reindex_content 需要 confirmGlobal:true。",
    inputSchema: siyuanAssetManageInputSchema,
    readOnly: false,
    inputHint: "action=rename/set_annotation/set_image_ocr/ocr/remove_unused_one/remove_unused_batch/full_reindex_content；set_annotation 的 path 必须是 PDF 或 .pdf.sya，使用 annotation 对象设置标注，或使用 clear:true 清除全部标注，不能同时传两者；remove_unused 路径必须包含 nb_agent/notebrain_agent/notebrain_test/notebrain-agent-test；full_reindex_content 需传 confirmGlobal:true。",
