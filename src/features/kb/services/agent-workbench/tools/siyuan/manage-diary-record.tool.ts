...
export function createManageDiaryRecordTool(
  ...
  return {
    ...
    source: "builtin",
    inputHint: "operation（必填）：add/update/delete；add 需要 categoryTitle 和 content；可传 tags、projectTargetId、isKeyRecord；关键记录必须关联项目。",
    boundary: "修改/删除前必须通过 query_diary_records 获取真实 recordId/headingBlockId/date；projectTargetId 必须属于当前项目容器；不编造 ID，不允许无项目关键记录。",
