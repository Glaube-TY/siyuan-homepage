...
export async function executeSiyuanBlockAttr(args: SiyuanBlockAttrInput): Promise<{ output: SiyuanToolOutput }> {
  ...
  switch (args.action) {
    ...
    case "set": {
      ...
      if (!args.attrs || Object.keys(args.attrs).length === 0) throw new SiyuanToolInvalidArgsError("attrs 不能为空。");
      await setBlockAttrsChecked(id, args.attrs);
      data = null;
      break;
    }
    case "batch_set": {
      if (!args.items || args.items.length === 0) throw new SiyuanToolInvalidArgsError("items 不能为空。");
