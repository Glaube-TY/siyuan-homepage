...
export function createSiyuanWorkspaceFileTool(deps: SiyuanWorkspaceFileDeps) {
  return createGenericSiyuanTool({
    ...
    title: "操作受限工作区文件",
    description: "在白名单目录内读取、写入、复制、重命名、删除文件或获取唯一文件名。",
    inputSchema: siyuanWorkspaceFileInputSchema,
    readOnly: false,
    inputHint: "action=read_dir/get_file/put_file/copy_file/rename_file/remove_file/unique_filename。",
    boundary: "只能访问 assets/templates/widgets/public/siyuan-homepage storage，拒绝系统路径和敏感目录。",
