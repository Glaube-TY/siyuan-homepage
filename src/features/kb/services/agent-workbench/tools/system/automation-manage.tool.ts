...
import { createAutomationJobId, automationTaskSchema, automationTriggerSchema } from "@/features/agent-platform/automation/automation-job-contract";
...
export interface AutomationManageToolOptions {
  source: { profileId: string; surface: string; conversationId?: string; messageId?: string };
...
const updateSchema = z.object({
  ...
  robotRouteRef: z.string().trim().min(1).max(500).nullable().optional(), enabled: z.boolean().optional(),
}).strict();
...
export function createAutomationManageTool(options: AutomationManageToolOptions): ToolContract {
  ...
  const create = action("automation_create", "创建 Agent 自动任务", "创建定时 Agent 或心跳任务。", createSchema, false, async (_ctx, args) => {
    const validationError = validateBackgroundTask(args.task);
    if (validationError) return result(false, null, validationError);
    const now = Date.now();
    const runnerDeviceId = (await options.resolveRunnerDeviceId?.())?.trim() || getNotificationDeviceId();
    const job = await automationJobStore.saveJob({
      ...
      runner: { deviceId: runnerDeviceId, runtime: "frontend", requiredCapabilities: [] },
      policy: { catchUp: "latest", overlap: "skip", maxRetries: 1, maxConsecutiveFailures: 3, expiresAfterMs: 86_400_000 },
      output: { ...(args.robotRouteRef ? { robotRouteRef: args.robotRouteRef } : {}) },
  ...
  const update = action("automation_update", "更新自动化", "更新任务定义或启用状态。", updateSchema, false, async (_ctx, args) => {
    ...
    const job = await automationJobStore.saveJob({
      ...
      ...(args.task ? { task: args.task } : {}), ...(args.robotRouteRef !== undefined ? { output: args.robotRouteRef ? { robotRouteRef: args.robotRouteRef } : {} } : {}),
  ...
  return createAggregateTool({
    name: "automation_manage", title: "Agent 自动化", description: "创建和管理定时 Agent 与心跳工作流。",
    boundary: "只管理需要 Agent 实际执行的工作流；普通提醒必须使用 notification_manage。所有运行都会持久化、受预算限制并写入运行记录。",
    actions: [
