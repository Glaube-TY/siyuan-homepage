...
const HOMEPAGE_SURFACE_SCHEMA = {
  type: "string",
  enum: ["desktop-homepage", "mobile-homepage"],
  description: "省略时按当前运行环境选择桌面或移动主页。",
};

function homepageObjectSchema(
  ...
  return {
    ...
    properties: { surface: HOMEPAGE_SURFACE_SCHEMA, ...properties },
    required,
  };
}

function homepageSurfaceArgsSchema(): Record<string, unknown> {
...
function homepageWriteActionMetadata(): AggregateActionMeta[] {
  ...
  return [
    ...
    { name: "create_section", title: "创建主页分栏", description: "创建桌面主页分栏。", readOnly: false, required: ["name", "expectedLayoutRevision"], argsSchema: homepageObjectSchema({ name: { type: "string", minLength: 1, maxLength: 60 }, sectionId: { type: "string", pattern: "^[A-Za-z0-9_-]+$" }, position: { type: "integer", minimum: 0 }, expectedLayoutRevision: { type: "integer", minimum: 1 } }, ["name", "expectedLayoutRevision"]), boundary: "仅 desktop-homepage 支持；修改 layout 布局结构。", notes: revisionNotes },
    { name: "rename_section", title: "重命名主页分栏", description: "重命名现有桌面主页分栏。", readOnly: false, required: ["sectionId", "name", "expectedSectionName", "expectedLayoutRevision"], argsSchema: homepageObjectSchema({ sectionId: { type: "string" }, name: { type: "string", minLength: 1, maxLength: 60 }, expectedSectionName: { type: "string" }, expectedLayoutRevision: { type: "integer", minimum: 1 } }, ["sectionId", "name", "expectedSectionName", "expectedLayoutRevision"]), notes: revisionNotes },
    { name: "reorder_sections", title: "排序主页分栏", description: "按完整 ID 列表调整桌面主页分栏顺序。", readOnly: false, required: ["orderedSectionIds", "expectedLayoutRevision"], argsSchema: homepageObjectSchema({ orderedSectionIds: { type: "array", items: { type: "string" }, minItems: 1 }, expectedLayoutRevision: { type: "integer", minimum: 1 } }, ["orderedSectionIds", "expectedLayoutRevision"]), notes: ["orderedSectionIds 必须包含全部现有分栏且不得重复。", ...revisionNotes] },
    { name: "remove_section", title: "删除主页分栏", description: "删除分栏结构并按现有邻接规则合并组件。", readOnly: false, required: ["sectionId", "expectedSectionName", "expectedWidgetCount", "expectedLayoutRevision"], argsSchema: homepageObjectSchema({ sectionId: { type: "string" }, expectedSectionName: { type: "string" }, expectedWidgetCount: { type: "integer", minimum: 0 }, expectedReceivingSectionId: { type: ["string", "null"] }, expectedLayoutRevision: { type: "integer", minimum: 1 } }, ["sectionId", "expectedSectionName", "expectedWidgetCount", "expectedLayoutRevision"]), boundary: "高风险；不会删除分栏中的组件或业务数据。", notes: ["expectedWidgetCount 来自 list_sections；可同时传 expectedReceivingSectionId 锁定接收分栏。", ...revisionNotes] },
    { name: "set_section_mode", title: "切换分栏模式", description: "开启或关闭桌面主页分栏模式，不删除分栏配置。", readOnly: false, required: ["enabled", "expectedLayoutRevision"], argsSchema: homepageObjectSchema({ enabled: { type: "boolean" }, expectedLayoutRevision: { type: "integer", minimum: 1 } }, ["enabled", "expectedLayoutRevision"]), notes: revisionNotes },
...
function homepageComponentBusinessActions(): AggregateActionMeta[] {
  const actionsByBusinessTool: Readonly<Record<string, readonly AggregateActionMeta[]>> = {
    ...
    homepage_accounting: [
      ...
      { name: "query_records", title: "查询记账流水", description: "按日期、方向、分类、账户和关键词限量查询流水。", readOnly: true, required: ["startDate/endDate 或 year"], argsSchema: { type: "object", additionalProperties: false, properties: { startDate: { type: "string", pattern: "^\\d{4}-\\d{2}-\\d{2}$" }, endDate: { type: "string", pattern: "^\\d{4}-\\d{2}-\\d{2}$" }, year: { type: "integer", minimum: 1970, maximum: 9999 }, direction: { type: "string", enum: ["expense", "income", "transfer"] }, category: { type: "string" }, account: { type: "string" }, keyword: { type: "string" }, limit: { type: "integer", minimum: 1, maximum: 200 }, includeArchived: { type: "boolean", description: "缺省 false 仅返回活动流水；true 同时返回归档流水，结果项带 archived 字段。" } } }, notes: ["不要默认查询全部历史；优先使用 summary 回答合计问题。"] },
      ...
      { name: "list_accounts", title: "列出资产账户", description: "列出资产账户；缺省仅活动账户，includeArchived=true 时包含归档账户并可区分状态。", readOnly: true, argsSchema: { type: "object", additionalProperties: false, properties: { includeArchived: { type: "boolean", description: "缺省 false 仅返回活动账户；true 同时返回归档账户，结果项带 archived 字段。" } } } },
...
function accountingRecordSchema(partial: boolean, includeDirection = true): Record<string, unknown> {
  const schema: Record<string, unknown> = {
    type: "object", additionalProperties: false,
    properties: {
      title: { type: "string", minLength: 1, maxLength: 200 }, direction: { type: "string", enum: ["expense", "income", "transfer"] }, amount: { type: "number", exclusiveMinimum: 0 }, date: { type: "string", pattern: "^\\d{4}-\\d{2}-\\d{2}$" }, categoryPrimary: { type: "string" }, categorySecondary: { type: "string" }, account: { type: "string", minLength: 1, description: "来源账户：活动账户的 accountId 或唯一精确名称，保存前规范化为 accountId。" }, counterAccount: { type: "string", minLength: 1, description: "转账目标账户：活动账户的 accountId 或唯一精确名称，保存前规范化为 accountId。" }, tags: { oneOf: [{ type: "string" }, { type: "array", items: { type: "string" } }] }, note: { type: "string", maxLength: 2000 }, currency: { type: "string" },
    },
  };
  if (!includeDirection) delete (schema.properties as Record<string, unknown>).direction;
  if (!partial) schema.required = ["title", "direction", "amount"];
  return schema;
}

function accountingAccountSchema(partial: boolean): Record<string, unknown> {
  const schema: Record<string, unknown> = { type: "object", additionalProperties: false, properties: { name: { type: "string", minLength: 1 }, type: { type: "string", minLength: 1 }, currency: { type: "string" }, openingBalance: { type: "number", minimum: 0 }, currentBalance: { type: "number", minimum: 0 }, sortOrder: { type: "number" }, note: { type: "string" } } };
  if (!partial) schema.required = ["name", "type"];
  return schema;
}

function expectedAccountingEntitySchema(idField: "recordId" | "accountId"): Record<string, unknown> {
  return { type: "object", additionalProperties: false, properties: { [idField]: { type: "string" }, expectedUpdatedAt: { type: "string" } }, required: [idField, "expectedUpdatedAt"] };
}

const EMPTY_ARGS_SCHEMA = { type: "object", additionalProperties: false, properties: {} };
const SIYUAN_BLOCK_ID_SCHEMA = { type: "string", pattern: "^\\d{14}-[a-z0-9]{7}$" };
const DATE_SCHEMA = { type: "string", pattern: "^\\d{4}-\\d{2}-\\d{2}$" };
const COUNTDOWN_EVENT_FIELDS_SCHEMA = {
  name: { type: "string", minLength: 1, maxLength: 200 },
  kind: { type: "string", enum: ["birthday", "anniversary", "deadline", "expiration", "milestone", "subscription", "custom"] },
  calendar: { type: "string", enum: ["solar", "lunar"] },
  recurrence: { type: "string", enum: ["none", "yearly"] },
  date: DATE_SCHEMA,
  lunarDate: { type: "object", additionalProperties: false, properties: { year: { type: "integer", minimum: 1900, maximum: 2200 }, month: { type: "integer", minimum: 1, maximum: 12 }, day: { type: "integer", minimum: 1, maximum: 30 }, isLeapMonth: { type: "boolean" } }, required: ["year", "month", "day"] },
  categoryId: { type: ["string", "null"] },
  tags: { type: "array", items: { type: "string", minLength: 1, maxLength: 40 }, maxItems: 50 },
  priority: { type: "string", enum: ["high", "normal", "low"] },
  note: { type: "string", maxLength: 2000 },
  linkedBlockId: SIYUAN_BLOCK_ID_SCHEMA,
  pastBehavior: { type: "string", enum: ["keep", "auto-archive"] },
};
const REVIEW_PLAN_FIELDS_SCHEMA = {
  nextDate: DATE_SCHEMA,
  note: { type: "string", maxLength: 2000 },
  category: { type: "string", maxLength: 100 },
  priority: { type: "string", enum: ["high", "medium", "low"] },
  plan: { type: "string", enum: ["manual", "ebbinghaus", "custom"] },
  intervals: { type: "array", items: { type: "integer", minimum: 0 }, maxItems: 50 },
};

function reviewTargetArgsSchema(
  properties: Record<string, unknown> = {},
  required: string[] = [],
): Record<string, unknown> {
  return {
    ...
    additionalProperties: false,
    properties: {
      targetId: SIYUAN_BLOCK_ID_SCHEMA,
      targetType: { type: "string", enum: ["doc", "block"] },
      ...properties,
    },
    required: ["targetId", "targetType", ...required],
  };
}
...
export const AGGREGATE_TOOL_CATALOG: AggregateToolMeta[] = [
  {
    ...
    title: "通知中心",
    description: "创建和管理固定内容的定时通知。",
    readOnly: false,
    requiresConfirmation: true,
    boundary: "通知只投递已经确定的内容；需要到点运行 Agent、读取数据或生成结果时使用 automation_manage。",
    actions: [
      { name: "list", title: "列出通知规则", description: "列出由 Agent 管理的通知规则。", readOnly: true },
      { name: "get", title: "查看通知规则", description: "读取通知规则。", readOnly: true },
      { name: "create", title: "创建通知规则", description: "创建一次性或周期通知。", readOnly: false },
      { name: "update", title: "更新通知规则", description: "修改内容、时间、渠道或状态。", readOnly: false },
      { name: "delete", title: "删除通知规则", description: "删除通知规则。", readOnly: false },
    ],
  },
  {
    name: "automation_manage",
    title: "Agent 自动化",
    description: "创建和管理定时 Agent 与变化监测。",
    readOnly: false,
    requiresConfirmation: true,
    ...
    actions: [
      ...
      { name: "create", title: "创建自动化", description: "创建定时 Agent 或监测任务。", readOnly: false },
  ...
  },
  {
    ...
    actions: [
      ...
      },
      {
        ...
        argsSchema: homepageSurfaceArgsSchema(), examples: [{ action: "get_layout", args: {} }],
      },
      {
        name: "list_sections", title: "列出主页分栏", description: "读取桌面主页分栏结构与布局顺序。", readOnly: true,
    ...
    actions: [
      ...
      },
      {
        ...
        examples: [
          ...
          { action: "extra_read", args: { action: "filter_sort", avID: "真实databaseId", blockID: "真实数据库块ID" } },
        ...
        notes: [
          ...
          "filter_sort 使用 avID + blockID；primary_key_values 使用 avID，并可选 keyword/page/pageSize。",
      ...
      },
      {
        ...
        required: ["action"],
        examples: [
          { action: "view", args: { action: "set_database_block_view", avID: "真实databaseId", blockID: "真实数据库块ID", viewID: "真实viewID" } },
          { action: "view", args: { action: "sort_key", avID: "真实databaseId", keyID: "真实keyId", previousKeyID: "真实keyId" } },
          { action: "view", args: { action: "change_layout", avID: "真实databaseId", blockID: "真实数据库块ID", layoutType: "table" } },
        ],
        notes: [
          ...DATABASE_ID_NOTES,
          ...
          "change_layout 使用 avID + blockID + layoutType（table/gallery/kanban），不是 viewID。",
          "set_group 使用 avID + blockID + 完整 group；group.field 是真实 keyId。传 group:null 会安全转为 removeAttrViewGroup 事务以清除分组。",
    ...
    actions: [
      ...
      },
      {
        ...
        examples: [
          ...
          { action: "block_ref", args: { action: "check_ref", ids: ["真实待检查块ID"] } },
          { action: "block_ref", args: { action: "get_def_ids_by_ref_text", anchor: "真实引用锚文本" } },
          { action: "block_ref", args: { action: "swap_ref", refID: "真实引用所在块ID", defID: "真实新定义块ID", includeChildren: false } },
          { action: "block_ref", args: { action: "transfer_ref", fromID: "真实原定义块ID", toID: "真实新定义块ID", refIDs: ["真实引用所在块ID"] } },
        ],
        notes: [
          "这里有两层 action：外层 action 固定为 block_ref，内层 args.action 才是引用操作。",
          "get_ref_ids/get_ref_text 的 id 是定义块或文档块 ID；check_ref 使用待检查块 ID 数组 ids。",
          "get_def_ids_by_ref_text 使用 anchor（引用锚文本），返回定义块候选。",
          "swap_ref 使用 refID（引用所在块 ID）+ defID（新定义块 ID）+ includeChildren。",
          "transfer_ref 使用 fromID（原定义块 ID）+ toID（新定义块 ID）+ refIDs（引用所在块 ID 数组）。",
          "不支持 get 这个内层 action。",
        ],
      },
      {
      ...
      },
      {
        name: "doc_transform",
        title: "转换文档结构",
        description: "执行文档转标题、标题转文档、列表项转文档；各类 ID 使用独立字段，不能混用。",
        readOnly: false,
        required: ["action"],
        examples: [
          { action: "doc_transform", args: { action: "doc_to_heading", sourceDocId: "源文档ID", targetBlockId: "目标块ID", after: true } },
          { action: "doc_transform", args: { action: "heading_to_doc", sourceHeadingId: "标题块ID", targetNotebookId: "目标笔记本ID", targetPath: "/目标父文档.sy" } },
          { action: "doc_transform", args: { action: "list_item_to_doc", sourceListItemId: "列表项块ID", targetNotebookId: "目标笔记本ID", targetPath: "/目标父文档.sy" } },
        ],
        notes: [
          ...
          "doc_to_heading 需要 sourceDocId、targetBlockId、after；after=true 表示放在目标块之后。",
          "heading_to_doc 需要 sourceHeadingId、targetNotebookId、targetPath；list_item_to_doc 需要 sourceListItemId、targetNotebookId、targetPath。",
          "转换到目标笔记本根目录时 targetPath 必须明确写 /，不能省略。",
          "targetPath/previousPath 是 .sy 存储路径，不是人类可读路径，也不是块 ID；toTop 是布尔值。",
    ...
    actions: [
      ...
      },
      {
        ...
        examples: [
          ...
          { action: "doc_tree", args: { action: "duplicate", id: "真实docId" } },
          { action: "doc_tree", args: { action: "sort", notebook: "真实notebookId", fromPaths: ["真实.sy存储路径"] } },
        ],
        notes: [
      ...
      },
      {
        ...
        argsSchema: {
          ...
          properties: {
            ...
            deckID: { type: "string", description: "add_cards/review/skip 必填；reset 当 resetType='deck' 时可省略（自动派生为 id）或传与 id 一致的值；list_cards/due_cards 可空（查全部）。" },
            ...
            id: { type: "string", description: "reset 必填目标 ID（当 resetType='deck' 时即为 deckID）。" },
        ...
        notes: [
          ...
          "reset 支持 resetType='deck'|'tree'|'notebook'；resetType='deck' 时 id 必填且为目标 deckID，deckID 可省略（内部自动派生 deckID=id）或传入与 id 一致的值，若 deckID 与 id 不一致会直接在调用前拒绝；blockIDs 可选，为空时重置该 Deck 全部卡片。",
          ...
          "当前 SiYuan Kernel 的 batchSetRiffCardsDueTime 仅在 builtin deck 中按 riffCardID 修改 Due；自定义 createRiffDeck 创建的 Deck 中独立 riffCardID 不属于该 API 的可靠支持范围（Kernel 对不在 builtin deck 中的 riffCardID 会静默跳过，返回 code=0 但 Due 不变）。因此：不要使用自建 disposable custom deck 验证 set_due_time 必须成功，是否生效必须以 verification / list_cards / get_card_info 回读为准，不能只看 code=0。若 verification.applied=false，Agent 不得反复重试相同调用。",
          "set_due_time 输出包含 kernelPayloadShape='cardDues[].due'、normalizedCardDues（保留完整 id）、kernelResult、needsVerification、verification（applied / partiallyApplied / appliedCount / details）与 warning；若未生效会明确提示 Kernel builtin deck 边界，测试报告必须以 list_cards/get_card_info 回读验证为准，applied=false 时严禁宣称写成功。",
          "测试 set_due_time 的正确流程：针对 builtin deck 中的卡片执行 set_due_time 并回读验证。不要在自建 custom deck 后强行断言 set_due_time 必须修改成功。",
