...
import { createWebSearchActionTool, type WebSearchToolDeps } from "../tools/web-search/web-search.tool";
...
export interface WebToolOptions {
  ...
  /** Robot Kernel exposes only web_fetch.read_page, never remote HTTP actions. */
  webFetchReadPageOnly?: boolean;
}
...
export function registerWebTools(
  ...
  if (options.globalToolAccess?.webFetch !== false) {
    const actions: AggregateActionBinding[] = [];
    if (options.webReadPageToolDeps) {
      actions.push({
        action: "read_page",
        tool: createReadPageActionTool(options.webReadPageToolDeps),
      });
    }
    if (!options.webFetchReadPageOnly) {
  ...
  if (options.globalToolAccess?.webSearch !== false && options.webSearchToolDeps) {
    toolRegistry.ensureTool(createWebSearchActionTool(options.webSearchToolDeps));
  }
}
