<script lang="ts">
  ...
  function startDrag(dom: DragEvent, event: GlobalCalendarEvent): void { if ((dom.target as Element).closest(".resize-handle")) { dom.preventDefault(); return; } dom.dataTransfer?.setData("text/calendar-event", event.id); if (dom.dataTransfer) dom.dataTransfer.effectAllowed = "move"; }
  ...
  function openTimedEvent(dom: MouseEvent, event: GlobalCalendarEvent): void { dom.stopPropagation(); if (Date.now() < suppressOpenUntil) return; if (event.editable && dom.detail !== 0) { const rect = (dom.currentTarget as HTMLElement).getBoundingClientRect(); const offset = dom.clientY - rect.top; if (offset <= resizeHandleHeight || offset >= rect.height - resizeHandleHeight) return; } onOpenEvent(event); }
  ...
  function resizeStart(dom: PointerEvent, event: GlobalCalendarEvent, edge: "start" | "end"): void {
    ...
    const up = () => { suppressOpenUntil = Date.now() + 300; cleanup(); const preview = resizePreview; void Promise.resolve().then(() => onResize(event, preview?.startTime ?? nextStart.toTimeString().slice(0, 5), preview?.endTime ?? nextEnd.toTimeString().slice(0, 5))).finally(() => { if (resizePreview?.id === event.id) resizePreview = null; }); };
...
<div class="time-grid" style={`--columns:${dates.length};--row-height:${rowHeight}px;--resize-handle-height:${resizeHandleHeight}px`}>
  ...
  {#each dates as date}
    <div class="day-column" role="group" aria-label={`${date} 时间安排`} onpointerdown={(event) => beginSelection(event, date)} onpointermove={updateSelection} onpointerup={finishSelection} onpointercancel={() => (selection = null)}>
      ...
      {#each timed.filter((event) => event.date === date) as event (event.id)}
        <button type="button" class="timed-event" class:compact={compactEvent(event)} class:resizing={resizePreview?.id === event.id} draggable={event.editable && resizePreview?.id !== event.id} style={eventStyle(event)} ondragstart={(dom) => startDrag(dom, event)} onclick={(dom) => openTimedEvent(dom, event)}>
...
<style>
  ...
  .resize-handle { position: absolute; z-index: 2; right: 0; left: 0; height: var(--resize-handle-height); cursor: ns-resize !important; touch-action: none; }
