...
<div class="chart-studio">
    <header>
        ...
        <div class="header-actions"><button type="button" class="primary" onclick={save} disabled={saving}>{saving ? "保存中…" : "保存图表"}</button><button type="button" class="icon" aria-label="关闭可视化图表工作台" onclick={onClose}><SiyuanIcon name="close" size={18} /></button></div>
    </header>
