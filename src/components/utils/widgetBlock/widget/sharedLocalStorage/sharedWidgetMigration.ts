...
export type MigrationStore = "focus" | "cybmok" | "countdown" | "fixed-assets" | "review-docs";
...
let migrationState: SharedWidgetMigrationState = { status: "idle", migratedStores: [] };

export function getSharedWidgetMigrationState(): SharedWidgetMigrationState {
    return { ...migrationState, migratedStores: [...migrationState.migratedStores] };
}
...
function createMetadata(
    ...
    const pendingDatabaseRows = Array.from(byDatabase, ([databaseId, values]) => ({
        databaseId,
        rowIds: values.map((row) => row.rowId),
        srcIds: values.map((row) => row.srcId),
    })).filter((item) => item.rowIds.length > 0);
    const hasCleanup = pendingDatabaseRows.length > 0 || legacyRootFiles.length > 0;
    return {
        version: 1,
        dataValidated: true,
        importedAt: nowIso(),
        ...
        pendingDatabaseRows: pendingDatabaseRows.length > 0 ? pendingDatabaseRows : undefined,
    };
}
...
async function readAllLegacyRows<T>(
    ...
    for (const databaseId of databaseIds) {
        try {
            ...
            console.warn(`[sharedWidgetMigration] 读取旧数据库 ${databaseId} 失败`, error);
            throw error;
        }
    }
    return { scans, rows };
...
function hasCompletedMigration(file: SharedRevisionedFile & { migration?: SharedWidgetMigrationMetadata } | null): boolean {
    return file?.migration?.dataValidated === true
        && (file.migration.status === "complete" || file.migration.status === "cleanup-pending");
}
...
function mergePendingDatabaseRows(
    ...
    for (const metadata of sources) {
        for (const group of metadata?.pendingDatabaseRows || []) {
            if (!byDatabase.has(group.databaseId)) byDatabase.set(group.databaseId, new Map());
...
export function mergeMigrationMetadata(
    ...
    const cleanupErrors = Array.from(new Set([existing?.cleanupError, incoming?.cleanupError].filter(Boolean)));
    return {
        ...metadata,
        dataValidated: true,
        importedAt: existing?.importedAt || incoming?.importedAt || nowIso(),
        sourceRecordCount: (existing?.sourceRecordCount || 0) + (incoming?.sourceRecordCount || 0),
        importedRecordCount: (existing?.importedRecordCount || 0) + (incoming?.importedRecordCount || 0),
        legacyDatabaseIds,
        legacyRootFiles,
        pendingDatabaseRows,
        status: cleanupPending ? "cleanup-pending" : "complete",
        cleanupStatus: cleanupPending ? "pending" : "complete",
        cleanupError: cleanupErrors.length > 0 ? cleanupErrors.join("；") : undefined,
    };
}
...
function rowsNotAlreadyPending<T>(
    ...
    for (const metadata of sources) {
        for (const group of metadata?.pendingDatabaseRows || []) {
            for (const rowId of group.rowIds) pending.add(`${group.databaseId}\u0000${rowId}`);
        }
    }
    return rows.filter((row) => !pending.has(`${row.databaseId}\u0000${row.rowId}`));
}
...
async function saveValidatedMigrationMetadata<T extends SharedRevisionedFile & { migration?: SharedWidgetMigrationMetadata }>(
    options: {
    ...
    await mutateSharedJson({
        ...
        mutate: (file) => { file.migration = options.metadata; },
        validate: (actual) => {
            const saved = actual.migration;
            if (saved?.dataValidated !== true || saved.importedAt !== options.metadata.importedAt
                ...
                || JSON.stringify(saved.legacyDatabaseIds) !== JSON.stringify(options.metadata.legacyDatabaseIds)
                || JSON.stringify(saved.legacyRootFiles) !== JSON.stringify(options.metadata.legacyRootFiles)
                || JSON.stringify(saved.pendingDatabaseRows || []) !== JSON.stringify(options.metadata.pendingDatabaseRows || [])) {
...
function legacyConfigFields(type: DatabaseWidgetType): string[] {
    return type === "focus" ? ["focusDatabaseId"]
        : type === "CYBMOK" ? ["CYBMOKDatabaseId", "cybmokDatabaseId"]
            : type === "countdown" ? ["countdownDatabaseId", "eventList"]
                : type === "fixedAssets" ? ["fixedAssetsDatabaseId"]
...
async function cleanWidgetConfigs(
    ...
    for (const record of configs.filter((item) => item.config.type === type)) {
        try {
            const latest = parseLegacyWidgetConfig(await plugin.loadData(record.path));
            ...
            if (latestDatabaseIds.some((databaseId) => !validatedDatabaseIds.has(databaseId))) {
                throw new Error("最新组件配置引用了尚未安全扫描的旧数据库");
            }
            if (type === "countdown" && "eventList" in latestData
            ...
            const reloaded = parseLegacyWidgetConfig(await plugin.loadData(record.path));
            if (!reloaded || reloaded.type !== type
...
async function cleanLegacyCountdownNotifySettings(validatedDatabaseIds: Set<string>): Promise<string[]> {
    const plugin = getSharedWidgetStoragePlugin();
    try {
        ...
        const databaseId = typeof latest.databaseId === "string" ? latest.databaseId.trim() : "";
        ...
        const reloaded = parseLegacyWidgetConfig(await plugin.loadData(COUNTDOWN_NOTIFY_SETTINGS_KEY));
        if (!reloaded || reloaded.version !== 2 || "databaseId" in reloaded || !sameJsonValue(reloaded, expected)) {
            throw new Error("纪念日通知设置清理后校验失败");
        }
        return [];
    } catch (error) {
        console.warn("[sharedWidgetMigration] 清理旧纪念日通知 databaseId 失败", error);
...
function widgetConfigNeedsCleanup(
    ...
    const fields = legacyConfigFields(type);
    return configs.some((record) => record.config.type === type
...
async function runCleanup<T extends SharedRevisionedFile & { migration?: SharedWidgetMigrationMetadata }>(options: {
    store: MigrationStore;
    path: string;
    ...
    if (!current?.migration || current.migration.dataValidated !== true || current.migration.status === "failed") return;
    if (current.migration.status === "complete" && current.migration.cleanupStatus === "complete") return;
    const metadata = current.migration;
    const pendingRows: NonNullable<SharedWidgetMigrationMetadata["pendingDatabaseRows"]> = [];
    const errors: string[] = [];
    ...
    for (const group of metadata.pendingDatabaseRows || []) {
        ...
        try {
            ...
            unrelatedBefore = (await getLegacyAttributeViewRowIds(group.databaseId))
                .filter((rowId) => !targetIds.has(rowId));
            await removeLegacyAttributeViewRows(group.databaseId, group.rowIds.map((rowId, index) => ({
                rowId,
        ...
        try {
            const allRemainingIds = await getLegacyAttributeViewRowIds(group.databaseId);
            const remainingSet = new Set(allRemainingIds);
            const remainingIds = group.rowIds.filter((rowId) => remainingSet.has(rowId));
            const missingUnrelated = unrelatedBefore.filter((rowId) => !remainingSet.has(rowId));
            if (missingUnrelated.length > 0) {
                throw new Error(`旧数据库清理验证发现非目标行缺失：${group.databaseId}`);
            }
            if (remainingIds.length > 0) {
                pendingRows.push({
                    databaseId: group.databaseId,
                    rowIds: remainingIds,
                    srcIds: remainingIds.map((rowId) => group.srcIds?.[group.rowIds.indexOf(rowId)] || rowId),
                });
            }
        } catch (error) {
    ...
    if (options.additionalCleanup) configFailures.push(...await options.additionalCleanup());
    if (configFailures.length > 0) errors.push(`组件配置清理失败：${configFailures.join("、")}`);
    const cleanupPending = pendingRows.length > 0 || pendingRootFiles.length > 0 || configFailures.length > 0;
    await mutateSharedJson({
        store: options.store,
        path: options.path,
        createEmpty: options.createEmpty,
        ...
        mutate: (file) => {
            ...
            file.migration = {
                ...
                status: cleanupPending ? "cleanup-pending" : "complete",
                cleanupStatus: cleanupPending ? "pending" : "complete",
                cleanupError: cleanupPending ? errors.join("；") || "旧数据清理待重试" : undefined,
        ...
        validate: (actual) => {
            const saved = actual.migration;
            if (!saved || saved.cleanupStatus !== (cleanupPending ? "pending" : "complete")
                || saved.status !== (cleanupPending ? "cleanup-pending" : "complete")
                || JSON.stringify(saved.legacyRootFiles) !== JSON.stringify(pendingRootFiles)
                || JSON.stringify(saved.pendingDatabaseRows || []) !== JSON.stringify(pendingRows)) {
...
async function migrateFocus(
    ...
    const existingIndex = await loadSharedJson(FOCUS_INDEX_FILE, normalizeFocusIndexFile);
    ...
    const current = await loadSharedJson(FOCUS_STATISTICS_FILE, normalizeFocusStatisticsFile);
    const currentTrustedMetadata = current?.migration?.dataValidated === true ? current.migration : undefined;
    const rootRaw = await readPluginData("widget-focus-statistics.json") as any;
    const currentNeedsImport = rootSourceNeedsImport(FOCUS_STATISTICS_FILE, Boolean(current), existingMetadata);
    const rootNeedsImport = rootSourceNeedsImport("widget-focus-statistics.json", rootRaw != null, existingMetadata);
    const databaseIdsToScan = Array.from(new Set(databaseIds));
    const needsImport = !hasCompletedMigration(existingIndex)
        || currentNeedsImport || rootNeedsImport || databaseIdsToScan.length > 0;
    ...
    if (!needsImport) {
        ...
        const baselineMismatch = baseline.totals.totalFocusTime !== existingIndex?.legacyTotals.totalFocusTime
            || baseline.totals.totalFocusTimes !== existingIndex?.legacyTotals.totalFocusTimes;
        if (baselineMismatch || sessionYears.some((year) => !existingIndex?.years.includes(year))) {
    ...
    if (needsImport) {
        const { rows: dbRows } = await readAllLegacyRows(databaseIdsToScan, readLegacyFocusDatabase);
        const newlyObservedRows = rowsNotAlreadyPending(dbRows, existingMetadata, currentTrustedMetadata);
        ...
        if (rootNeedsImport) candidates.push({
        ...
        const legacyTotals = candidates.reduce<FocusLegacyTotals>((result, item) => ({
            totalFocusTime: Math.max(result.totalFocusTime, Math.max(0, Number(item.totalFocusTime) || 0)),
            totalFocusTimes: Math.max(
        ...
        const delta = createMetadata(
            (currentNeedsImport && !currentTrustedMetadata ? 1 : 0) + (rootNeedsImport ? 1 : 0) + newlyObservedRows.length,
            (newlyObservedRows.length > 0 ? 1 : 0)
                + (currentNeedsImport && !currentTrustedMetadata ? 1 : 0) + (rootNeedsImport ? 1 : 0),
            databaseIdsToScan,
            newRootFiles,
            dbRows,
        );
        ...
        await rebuildFocusIndexFromFiles({ legacyTotals: baseline.totals, migration: existingMetadata, dispatch: false });
        await saveValidatedMigrationMetadata({
            store: "focus", path: FOCUS_INDEX_FILE,
            createEmpty: createEmptyFocusIndexFile, normalize: normalizeFocusIndexFile, metadata,
        });
    } else if (existingMetadata && widgetConfigNeedsCleanup(configs, "focus")) {
        await saveValidatedMigrationMetadata({
            store: "focus", path: FOCUS_INDEX_FILE,
            createEmpty: createEmptyFocusIndexFile, normalize: normalizeFocusIndexFile,
            metadata: includePendingConfigCleanup(existingMetadata, configs, "focus"),
        });
    }
    await runCleanup<FocusIndexFile>({
        store: "focus", path: FOCUS_INDEX_FILE, widgetType: "focus", configs,
        createEmpty: createEmptyFocusIndexFile, normalize: normalizeFocusIndexFile,
...
function parseLegacyCYBMOKRoot(raw: unknown): CYBMOKRecord[] {
    ...
    const now = nowIso();
    return Object.entries(raw as Record<string, unknown>).map(([dateValue, countValue]) => ({
        date: toLocalCYBMOKDate(dateValue), count: Math.max(0, Number(countValue) || 0), createdAt: now, updatedAt: now,
...
function mergeCYBMOKSources(sources: CYBMOKRecord[][]): CYBMOKRecord[] {
    ...
    for (const source of sources) {
        ...
        for (const item of source) {
            ...
            else sourceTotals.set(date, { ...item, date, count: Math.max(0, Number(item.count) || 0) });
        }
...
async function loadExistingCYBMOKBatches(years: number[]): Promise<CYBMOKBatchRecord[]> {
...
async function migrateCYBMOK(databaseIds: string[], configs: LegacyWidgetConfigRecord[]): Promise<boolean> {
    const existingIndex = await loadSharedJson(CYBMOK_INDEX_FILE, normalizeCYBMOKIndexFile);
    const existingMetadata = existingIndex?.migration;
    const batchYears = await listCYBMOKBatchYears();
    assertSharedWidgetYearFilesComplete(existingIndex?.years || [], batchYears);
    const current = await loadSharedJson(CYBMOK_RECORDS_FILE, normalizeCYBMOKRecordsFile);
    const currentTrustedMetadata = current?.migration?.dataValidated === true ? current.migration : undefined;
    const rootRaw = await readPluginData("CYBMOKData.json");
    const currentNeedsImport = rootSourceNeedsImport(CYBMOK_RECORDS_FILE, Boolean(current), existingMetadata);
    const rootNeedsImport = rootSourceNeedsImport("CYBMOKData.json", rootRaw != null, existingMetadata);
    const databaseIdsToScan = Array.from(new Set(databaseIds));
    const needsImport = !hasCompletedMigration(existingIndex)
        || currentNeedsImport || rootNeedsImport || databaseIdsToScan.length > 0;
    if (!needsImport && batchYears.some((year) => !existingIndex?.years.includes(year))) {
        await rebuildCYBMOKIndexFromFiles({ migration: existingMetadata, dispatch: false });
    }
    if (needsImport) {
        ...
        const runtimeBatches = existingBatches.filter((batch) => batch.kind === "batch");
        const existingLegacyRecords = Array.from(existingBatches
            .filter((batch) => batch.kind === "legacy-daily")
            .reduce((totals, batch) => totals.set(batch.localDate, (totals.get(batch.localDate) || 0) + batch.count), new Map<string, number>()),
        ([date, count]) => ({ date, count, createdAt: "", updatedAt: "" }));
        const { rows: dbRows } = await readAllLegacyRows(databaseIdsToScan, readLegacyCYBMOKDatabase);
        const newlyObservedRows = rowsNotAlreadyPending(dbRows, existingMetadata, currentTrustedMetadata);
        const databaseSources = databaseIdsToScan.map((databaseId) => dbRows
        ...
        const delta = createMetadata(
            (currentNeedsImport && !currentTrustedMetadata ? current?.records.length || 0 : 0)
                + rootRecords.length + newlyObservedRows.length,
        ...
        const batches: CYBMOKBatchRecord[] = [
            ...runtimeBatches,
            ...records.filter((record) => record.count > 0)
        ...
        await saveValidatedMigrationMetadata({
            store: "cybmok", path: CYBMOK_INDEX_FILE,
            createEmpty: createEmptyCYBMOKIndexFile, normalize: normalizeCYBMOKIndexFile, metadata,
        });
    } else if (existingMetadata && widgetConfigNeedsCleanup(configs, "CYBMOK")) {
        await saveValidatedMigrationMetadata({
            store: "cybmok", path: CYBMOK_INDEX_FILE,
            createEmpty: createEmptyCYBMOKIndexFile, normalize: normalizeCYBMOKIndexFile,
            metadata: includePendingConfigCleanup(existingMetadata, configs, "CYBMOK"),
        });
    }
    await runCleanup<CYBMOKIndexFile>({
        store: "cybmok", path: CYBMOK_INDEX_FILE, widgetType: "CYBMOK", configs,
        createEmpty: createEmptyCYBMOKIndexFile, normalize: normalizeCYBMOKIndexFile,
        validatedDatabaseIds: databaseIdsToScan,
    });
    return needsImport;
...
function mergeCountdownSources(sources: Array<{ events: CountdownEventInput[]; priority: number }>): CountdownEventRecord[] {
    ...
    for (const source of sources) {
        for (const input of source.events) {
            ...
            const natural = `${input.name.trim()}|${input.date.trim()}|${Boolean(input.anniversary)}`;
            const id = input.id?.trim() || `countdown-legacy-${hashText(natural)}`;
            const now = nowIso();
            const event: CountdownEventRecord = {
                id, name: input.name.trim(), date: input.date.trim(), anniversary: Boolean(input.anniversary),
                order: Number.isFinite(Number(input.order)) ? Number(input.order) : fallbackOrder++,
                createdAt: input.createdAt || now, updatedAt: input.updatedAt || "", archived: input.archived === true,
            };
            const idKey = input.id?.trim() ? `id:${id}` : "";
            const naturalKey = `natural:${natural}`;
            const key = (idKey ? ids.get(idKey) : undefined) || naturals.get(naturalKey) || idKey || naturalKey;
            const current = map.get(key);
            if (!current || compareTimestamp(event.updatedAt, current.event.updatedAt) > 0
                || (compareTimestamp(event.updatedAt, current.event.updatedAt) === 0 && source.priority > current.priority)) {
    ...
    return Array.from(map.values(), (item) => item.event)
        ...
        .map((event, order) => ({ ...event, order, updatedAt: event.updatedAt || event.createdAt }));
}

function collectWidgetCountdownEvents(configs: LegacyWidgetConfigRecord[]): CountdownEventInput[] {
    return configs.filter((record) => record.config.type === "countdown")
        .flatMap((record) => Array.isArray(record.config.data?.eventList) ? record.config.data.eventList : []);
}
...
async function migrateCountdown(
    ...
    const existing = await loadSharedJson(COUNTDOWN_EVENTS_FILE, normalizeCountdownEventsFile);
    ...
    const needsImport = !hasCompletedMigration(existing) || databaseIdsToScan.length > 0 || widgetEvents.length > 0;
    if (needsImport) {
        const { rows: dbRows } = await readAllLegacyRows(databaseIdsToScan, readLegacyCountdownDatabase);
        ...
        const metadata = includePendingConfigCleanup(
            mergeMigrationMetadata(
                existingMetadata,
                createMetadata(
                    Math.max(0, events.length - (existing?.events.length || 0)) + newlyObservedRows.length,
        ...
        await mutateSharedJson({
            store: "countdown", path: COUNTDOWN_EVENTS_FILE,
            createEmpty: createEmptyCountdownEventsFile, normalize: normalizeCountdownEventsFile,
            mutate: (file) => { file.events = events; },
            validate: (actual) => validateCountdownEventRecords(
                actual.events,
                events,
                "纪念日迁移业务数据校验失败",
            ),
        ...
        await saveValidatedMigrationMetadata({
            store: "countdown", path: COUNTDOWN_EVENTS_FILE,
            createEmpty: createEmptyCountdownEventsFile, normalize: normalizeCountdownEventsFile, metadata,
        });
    } else if (existingMetadata && widgetConfigNeedsCleanup(configs, "countdown")) {
        await saveValidatedMigrationMetadata({
            store: "countdown", path: COUNTDOWN_EVENTS_FILE,
            createEmpty: createEmptyCountdownEventsFile, normalize: normalizeCountdownEventsFile,
            metadata: includePendingConfigCleanup(existingMetadata, configs, "countdown"),
        });
    }
    await runCleanup<CountdownEventsFile>({
        store: "countdown", path: COUNTDOWN_EVENTS_FILE, widgetType: "countdown", configs,
        createEmpty: createEmptyCountdownEventsFile, normalize: normalizeCountdownEventsFile,
        validatedDatabaseIds: databaseIdsToScan,
...
function normalizeFixedAssetForMigration(input: FixedAssetRecord, migrationNow: string): FixedAssetRecord {
    ...
    return {
        ...input,
        createdAt,
        updatedAt: typeof input.updatedAt === "string" && input.updatedAt.trim() ? input.updatedAt : createdAt,
        archived: input.archived === true,
    };
}

function mergeFixedAssets(sources: Array<{ assets: FixedAssetRecord[]; priority: number }>): FixedAssetRecord[] {
    ...
    for (const source of sources) {
        for (const input of source.assets) {
            ...
            const key = (idKey ? ids.get(idKey) : undefined) || naturals.get(naturalKey) || idKey || naturalKey;
            const current = map.get(key);
            if (!current || compareTimestamp(asset.updatedAt, current.asset.updatedAt) > 0
                || (compareTimestamp(asset.updatedAt, current.asset.updatedAt) === 0 && source.priority > current.priority)) {
...
async function migrateFixedAssets(databaseIds: string[], configs: LegacyWidgetConfigRecord[]): Promise<boolean> {
    const existing = await loadSharedJson(FIXED_ASSETS_FILE, normalizeFixedAssetsFile);
    const existingMetadata = existing?.migration;
    const databaseIdsToScan = Array.from(new Set(databaseIds));
    const needsImport = !hasCompletedMigration(existing) || databaseIdsToScan.length > 0;
    if (needsImport) {
        const { rows: dbRows } = await readAllLegacyRows(databaseIdsToScan, readLegacyFixedAssetsDatabase);
        const newlyObservedRows = rowsNotAlreadyPending(dbRows, existingMetadata);
        const migrationNow = nowIso();
        const assets = mergeFixedAssets([
            { assets: dbRows.map((row) => normalizeFixedAssetForMigration(row.data, migrationNow)), priority: 2 },
            { assets: (existing?.assets || []).map((asset) => normalizeFixedAssetForMigration(asset, migrationNow)), priority: 3 },
        ]);
        const metadata = includePendingConfigCleanup(
            mergeMigrationMetadata(
                existingMetadata,
        ...
        await mutateSharedJson({
            store: "fixed-assets", path: FIXED_ASSETS_FILE,
            createEmpty: createEmptyFixedAssetsFile, normalize: normalizeFixedAssetsFile,
            mutate: (file) => { file.assets = assets; },
            validate: (actual) => validateFixedAssetRecords(
                actual.assets,
                assets,
                "固定资产迁移业务数据校验失败",
            ),
        ...
        await saveValidatedMigrationMetadata({
            store: "fixed-assets", path: FIXED_ASSETS_FILE,
            createEmpty: createEmptyFixedAssetsFile, normalize: normalizeFixedAssetsFile, metadata,
        });
    } else if (existingMetadata && widgetConfigNeedsCleanup(configs, "fixedAssets")) {
        await saveValidatedMigrationMetadata({
            store: "fixed-assets", path: FIXED_ASSETS_FILE,
            createEmpty: createEmptyFixedAssetsFile, normalize: normalizeFixedAssetsFile,
            metadata: includePendingConfigCleanup(existingMetadata, configs, "fixedAssets"),
        });
    }
    await runCleanup<FixedAssetsFile>({
        store: "fixed-assets", path: FIXED_ASSETS_FILE, widgetType: "fixedAssets", configs,
        createEmpty: createEmptyFixedAssetsFile, normalize: normalizeFixedAssetsFile,
        validatedDatabaseIds: databaseIdsToScan,
    });
    return needsImport;
...
function logCompleteness(log: ReviewLogEntry): number {
    return Object.values(log).filter((value) => value !== "" && value !== null && value !== undefined).length;
}
...
function mergeReviewLogs(sources: ReviewLogMigrationItem[][]): ReviewLogMigrationItem[] {
    const map = new Map<string, { item: ReviewLogMigrationItem; priority: number }>();
    sources.forEach((items, priority) => {
        for (const item of items) {
            const key = getReviewLogDedupeKey(item.log);
            const current = map.get(key);
            if (!current || logCompleteness(item.log) > logCompleteness(current.item.log)
                || (logCompleteness(item.log) === logCompleteness(current.item.log) && priority > current.priority)) {
...
async function loadExistingReviewLogs(years: number[]): Promise<{ years: number[]; items: ReviewLogMigrationItem[] }> {
...
async function migrateReviewDocs(databaseIds: string[], configs: LegacyWidgetConfigRecord[]): Promise<boolean> {
    const existingIndex = await loadSharedJson(REVIEW_LOG_INDEX_FILE, normalizeReviewLogIndexFile);
    const existingMetadata = existingIndex?.migration;
    const logYears = await listReviewLogYears();
    assertSharedWidgetYearFilesComplete(existingIndex?.years || [], logYears);
    const databaseIdsToScan = Array.from(new Set(databaseIds));
    const needsImport = !hasCompletedMigration(existingIndex) || databaseIdsToScan.length > 0;
    if (!needsImport && logYears.some((year) => !existingIndex?.years.includes(year))) {
        await rebuildReviewLogIndexFromFiles();
    }
    if (needsImport) {
        const existingLogs = await loadExistingReviewLogs(logYears);
        const { rows: dbRows } = await readAllLegacyRows(databaseIdsToScan, readLegacyReviewDocsDatabase);
        const newlyObservedRows = rowsNotAlreadyPending(dbRows, existingMetadata);
        const currentLocalYear = new Date().getFullYear();
        const logs = mergeReviewLogs([
            dbRows.map((row) => ({ log: row.data, sourceYear: getReviewLogYear(row.data, currentLocalYear) })),
            existingLogs.items,
        ]);
        ...
        const years = Array.from(new Set([...existingLogs.years, ...byYear.keys()])).sort();
        const yearCounts = Object.fromEntries(years.map((year) => [String(year), byYear.get(year)?.length || 0]));
        const metadata = includePendingConfigCleanup(
            mergeMigrationMetadata(
                existingMetadata,
                createMetadata(
        ...
        );
        {
            for (const year of years) {
                ...
                await mutateSharedJson({
                    store: "review-docs", path: getReviewLogsFile(year),
                    createEmpty: () => createEmptyReviewLogsYearFile(year),
                    normalize: (raw) => normalizeReviewLogsYearFile(raw, year),
                    mutate: (file) => { file.logs = yearLogs; },
                    validate: (actual) => validateReviewLogRecords(
                        actual.logs,
                        yearLogs,
            ...
            let savedTotal = 0;
            for (const year of years) {
                const saved = await loadSharedJson(
                ...
                for (const log of savedLogs) {
                    ...
                    if (allSavedKeys.has(key)) throw new Error(`复习日志迁移后仍存在跨年度重复：${key}`);
            ...
            await mutateSharedJson({
                store: "review-docs", path: REVIEW_LOG_INDEX_FILE,
                createEmpty: createEmptyReviewLogIndexFile, normalize: normalizeReviewLogIndexFile,
                mutate: (index) => {
                    index.years = years; index.yearCounts = yearCounts; index.totalLogs = savedTotal;
                },
                validate: (actual) => {
                    if (actual.totalLogs !== savedTotal || actual.years.length !== years.length
                        || years.some((year, index) => actual.years[index] !== year
            ...
            await saveValidatedMigrationMetadata({
                store: "review-docs", path: REVIEW_LOG_INDEX_FILE,
                createEmpty: createEmptyReviewLogIndexFile, normalize: normalizeReviewLogIndexFile, metadata,
            });
        }
    } else if (existingMetadata && widgetConfigNeedsCleanup(configs, "reviewDocs")) {
        await saveValidatedMigrationMetadata({
            store: "review-docs", path: REVIEW_LOG_INDEX_FILE,
            createEmpty: createEmptyReviewLogIndexFile, normalize: normalizeReviewLogIndexFile,
            metadata: includePendingConfigCleanup(existingMetadata, configs, "reviewDocs"),
        });
    }
    await runCleanup<ReviewLogIndexFile>({
        store: "review-docs", path: REVIEW_LOG_INDEX_FILE, widgetType: "reviewDocs", configs,
        createEmpty: createEmptyReviewLogIndexFile, normalize: normalizeReviewLogIndexFile,
        validatedDatabaseIds: databaseIdsToScan,
    });
    return needsImport;
...
async function runMigration(): Promise<void> {
    ...
    const notifyRaw = await readPluginData("countdownNotifySettings.json") as any;
    ...
    const tasks: Array<[MigrationStore, () => Promise<boolean>]> = [
        ["focus", () => runSharedWidgetExclusive(
            FOCUS_STORE_TRANSACTION_LOCK,
            () => migrateFocus(databaseIds.focus, configs),
        )],
        ["cybmok", () => runSharedWidgetExclusive(
            CYBMOK_STORE_TRANSACTION_LOCK,
            () => migrateCYBMOK(databaseIds.CYBMOK, configs),
        )],
        ["countdown", () => runSharedWidgetExclusive(
            COUNTDOWN_STORE_TRANSACTION_LOCK,
            () => migrateCountdown(
        ...
        ["fixed-assets", () => runSharedWidgetExclusive(
            FIXED_ASSETS_STORE_TRANSACTION_LOCK,
            () => migrateFixedAssets(databaseIds.fixedAssets, configs),
        )],
        ["review-docs", () => runSharedWidgetExclusive(
            REVIEW_DOCS_STORE_TRANSACTION_LOCK,
    ...
    for (const [store, migrate] of tasks) {
        try {
            ...
            failedStores[store] = error instanceof Error ? error.message : String(error);
            console.error(`[sharedWidgetMigration] ${store} 历史数据迁移失败详情`, error);
        }
    }
    const failed = Object.keys(failedStores).length > 0;
    migrationState = {
        status: failed ? "failed" : "complete",
        error: failed ? "部分组件历史数据迁移未完成，旧数据未删除，请重新加载插件后重试。" : undefined,
        migratedStores,
        failedStores: failed ? failedStores : undefined,
    };
    if (failed) {
        console.warn("[sharedWidgetMigration] 部分组件历史数据迁移未完成，旧数据未删除，请重新加载插件后重试。");
    }
}

export async function assertSharedWidgetMigrationReady(store: MigrationStore): Promise<void> {
...
export function ensureLegacySharedWidgetMigration(plugin?: any): Promise<void> {
    ...
    if (!migrationPromise) {
        migrationPromise = runSharedWidgetExclusive(SHARED_WIDGET_MIGRATION_LOCK, runMigration).catch((error) => {
            migrationState = {
                status: "failed",
                error: error instanceof Error ? error.message : String(error),
                migratedStores: [...migrationState.migratedStores],
            };
            console.warn("[sharedWidgetMigration] 部分组件历史数据迁移未完成，旧数据未删除", error);
            throw error;
        });
    }
