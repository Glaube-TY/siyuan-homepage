...
import type { CountdownEventRecord } from "../countdown/countdownData";
...
export function assertLegacyDatabaseScanSafe<T>(scan: LegacyDatabaseScanResult<T>): void {
    if (scan.totalRowCount > 0 && (!scan.schemaMatched || scan.recognizedRowCount === 0)) {
        throw new Error(`旧数据库存在记录，但无法确认组件数据结构，旧数据和配置均未删除：${scan.databaseId}`);
    }
}
...
const ALIASES = {
    ...
    fixedAssets: {
        ...
        category: ["category", "分类"], icon: ["icon", "图标"],
        purchasePrice: ["purchasePrice", "购买价格", "价格"],
        extraCost: ["extraCost", "附加成本", "附加项"],
        purchaseDate: ["purchaseDate", "购买日期", "购入日期"],
        retireDate: ["retireDate", "退役日期"], warrantyDate: ["warrantyDate", "过保日期"],
        expectedDays: ["expectedDays", "预计天数", "预计使用天数"],
        costMode: ["costMode", "均价方式", "计算均价方式"], note: ["note", "备注"],
        createdAt: ["createdAt", "创建时间"], updatedAt: ["updatedAt", "更新时间"],
        archived: ["archived", "已归档", "归档", "已删除"],
    },
    reviewDocs: {
        logId: ["logId", "日志ID", "记录ID"], reviewId: ["reviewId", "复习计划ID"],
        targetId: ["targetId", "目标块ID", "块ID"], targetRootId: ["targetRootId", "所属文档ID", "文档ID"],
        targetType: ["targetType", "目标类型"], targetTitle: ["targetTitle", "标题快照"],
        targetPath: ["targetPath", "路径快照"], action: ["action", "操作"], actionAt: ["actionAt", "操作时间"],
        previousDueDate: ["previousDueDate", "原复习日期"], nextDueDate: ["nextDueDate", "下次复习日期"],
        reviewCountBefore: ["reviewCountBefore", "原复习次数"], reviewCountAfter: ["reviewCountAfter", "复习次数"],
        intervalIndexBefore: ["intervalIndexBefore", "原间隔索引"], intervalIndexAfter: ["intervalIndexAfter", "间隔索引"],
        plan: ["plan", "计划类型"], intervals: ["intervals", "间隔配置"], category: ["category", "分类"],
        priority: ["priority", "优先级"], note: ["note", "备注"], createdAt: ["createdAt", "创建时间"],
...
function normalizeRawKeyValue(item: any): AttributeViewKeyValue | null {
    ...
    return { key: { id: key.id, name: key.name, type: key.type || item?.type || "text" }, values: item?.values || [] };
}
...
function normalizeSchema(raw: any): AttributeViewKeyValue[] {
    const source =
        ...
        || [];
    return source.map(normalizeRawKeyValue)
        .filter((item): item is AttributeViewKeyValue => item !== null);
}

async function loadAttributeViewReadOnly(databaseId: string): Promise<AttributeView | null> {
    ...
    const keyValues = schema.map((key) => ({
        ...key,
        values: av.keyValues.find((item) => item.key.id === key.key.id)?.values || key.values || [],
    }));
    for (const key of av.keyValues) if (!keyValues.some((item) => item.key.id === key.key.id)) keyValues.push(key);
    return { ...av, keyValues };
}
...
function groupRows(av: AttributeView): LegacyRow[] {
    ...
    for (const keyValue of av.keyValues) {
        for (const value of keyValue.values || []) {
            const rowId = value?.itemID || value?.itemId || value?.rowID || value?.rowId
                || value?.blockID || value?.blockId || value?.id || "";
            if (!rowId) continue;
            const srcId = value?.boundBlockID || value?.boundBlockId || value?.blockID || value?.blockId || rowId;
            if (!rows.has(rowId)) rows.set(rowId, { rowId, srcId, values: new Map() });
...
function extractArrayContent(value: unknown, joinValues: boolean): string {
    ...
    const contents = value.map((item) => stringContent(item?.content ?? item?.name ?? item?.value))
        .map((item) => item.trim())
        .filter(Boolean);
...
function extractText(value: any, joinValues = false): string {
    ...
    if (value.date?.isNotEmpty === false || value.number?.isNotEmpty === false) return "";
    const scalar = value.text?.content ?? value.block?.content ?? value.number?.content ?? value.date?.content;
    ...
    const singleSelect = Array.isArray(value.sSelect)
        ...
        : stringContent(value.sSelect?.content ?? value.sSelect?.name ?? value.sSelect?.value);
    if (singleSelect) return singleSelect;
    const assets = extractArrayContent(value.mAsset, joinValues);
    if (assets) return assets;
    if (typeof value.checkbox?.checked === "boolean") return String(value.checkbox.checked);
...
function normalizeDateCellText(value: string, mode: LegacyDateMode): string {
    ...
    if (mode === "local-date" && compact) return `${compact[1]}-${compact[2]}-${compact[3]}`;
    if (mode === "local-date" && /^\d{4}-\d{2}-\d{2}/.test(text)) return text.slice(0, 10);
...
function findKey(av: AttributeView, aliases: readonly string[], preferBlock = false): AttributeViewKeyValue | null {
    if (preferBlock) {
        const primary = av.keyValues.find((item) => item.key.type === "block");
        if (primary) return primary;
    }
    const names = aliases.map(normalizeName);
    return av.keyValues.find((item) => names.includes(normalizeName(item.key.name))) || null;
}
...
function createReader(av: AttributeView, row: LegacyRow) {
    return (
        ...
        return key.key.type === "date" ? normalizeDateCellText(text, dateMode) : text;
    };
}

function truthy(value: string): boolean {
    return ["1", "true", "yes", "是", "周年", "已归档", "归档", "已删除"].includes(value.trim().toLowerCase());
}
...
function createStableLegacyReviewLogId(databaseId: string, rowId: string): string {
...
async function readRows<T>(
    ...
    const totalRowCount = Math.max(
        groupedRows.length,
        ...av.keyValues.map((keyValue) => Array.isArray(keyValue.values) ? keyValue.values.length : 0),
    );
    const rows = groupedRows.map((row) => {
        const data = parse(av, row);
        return data ? { databaseId, rowId: row.rowId, srcId: row.srcId, data } : null;
...
export function readLegacyFocusDatabase(databaseId: string): Promise<LegacyDatabaseScanResult<FocusStatistics>> {
    return readRows(databaseId, (av) => Boolean(
...
export function readLegacyCYBMOKDatabase(databaseId: string): Promise<LegacyDatabaseScanResult<CYBMOKRecord>> {
    return readRows(databaseId, (av) => Boolean(
        findKey(av, ALIASES.cybmok.count)
        && (findKey(av, ALIASES.cybmok.date) || findKey(av, ALIASES.cybmok.title, true)),
    ), (av, row) => {
        const read = createReader(av, row);
        const date = read(ALIASES.cybmok.date, false, false, "local-date") || read(ALIASES.cybmok.title, true);
...
export function readLegacyCountdownDatabase(databaseId: string): Promise<LegacyDatabaseScanResult<CountdownEventRecord>> {
    return readRows(databaseId, (av) => Boolean(
        findKey(av, ALIASES.countdown.date)
        && (findKey(av, ALIASES.countdown.name) || findKey(av, ALIASES.countdown.title, true)),
    ), (av, row) => {
        const read = createReader(av, row);
        const name = read(ALIASES.countdown.name) || read(ALIASES.countdown.title, true);
...
export function readLegacyFixedAssetsDatabase(databaseId: string): Promise<LegacyDatabaseScanResult<FixedAssetRecord>> {
    return readRows(databaseId, (av) => Boolean(
        ...
        const purchaseDate = read(ALIASES.fixedAssets.purchaseDate, false, false, "local-date");
        ...
        return {
            ...
            name, category: read(ALIASES.fixedAssets.category), icon: read(ALIASES.fixedAssets.icon) || "📦",
            purchasePrice: numberValue(read(ALIASES.fixedAssets.purchasePrice)),
            extraCost: numberValue(read(ALIASES.fixedAssets.extraCost)), purchaseDate,
            retireDate: read(ALIASES.fixedAssets.retireDate, false, false, "local-date") || undefined,
            warrantyDate: read(ALIASES.fixedAssets.warrantyDate, false, false, "local-date") || undefined,
            expectedDays: numberValue(read(ALIASES.fixedAssets.expectedDays)) || undefined,
            costMode: mode === "expectedLife" || mode === "retireDate" ? mode : "elapsed",
...
export function readLegacyReviewDocsDatabase(databaseId: string): Promise<LegacyDatabaseScanResult<ReviewLogEntry>> {
    return readRows(databaseId, (av) => Boolean(
        ...
        if (!targetId || !["create", "review", "postpone", "update", "finish", "remove"].includes(action)) return null;
        return {
            logId: read(ALIASES.reviewDocs.logId) || createStableLegacyReviewLogId(databaseId, row.rowId),
            reviewId: read(ALIASES.reviewDocs.reviewId), targetId,
            targetRootId: read(ALIASES.reviewDocs.targetRootId),
            targetType: read(ALIASES.reviewDocs.targetType) === "doc" ? "doc" : "block",
            targetTitle: read(ALIASES.reviewDocs.targetTitle), targetPath: read(ALIASES.reviewDocs.targetPath),
            action: action as ReviewLogEntry["action"],
            actionAt: read(ALIASES.reviewDocs.actionAt, false, false, "iso"),
            previousDueDate: read(ALIASES.reviewDocs.previousDueDate, false, false, "local-date"),
            nextDueDate: read(ALIASES.reviewDocs.nextDueDate, false, false, "local-date"),
            reviewCountBefore: numberValue(read(ALIASES.reviewDocs.reviewCountBefore)),
            reviewCountAfter: numberValue(read(ALIASES.reviewDocs.reviewCountAfter)),
            intervalIndexBefore: numberValue(read(ALIASES.reviewDocs.intervalIndexBefore)),
            intervalIndexAfter: numberValue(read(ALIASES.reviewDocs.intervalIndexAfter)),
            plan: read(ALIASES.reviewDocs.plan), intervals: read(ALIASES.reviewDocs.intervals, false, true),
            category: read(ALIASES.reviewDocs.category), priority: read(ALIASES.reviewDocs.priority),
...
export async function verifyLegacyRowsRemoved(databaseId: string, rowIds: string[]): Promise<void> {
    const av = await loadAttributeViewReadOnly(databaseId);
    ...
    if (notRemoved.length > 0) throw new Error(`旧数据库行删除校验失败：${databaseId}`);
}

export async function getRemainingLegacyRowIds(databaseId: string, rowIds: string[]): Promise<string[]> {
    const av = await loadAttributeViewReadOnly(databaseId);
    if (!av) throw new Error(`旧数据库清理后无法重新读取：${databaseId}`);
    const remaining = new Set(groupRows(av).map((row) => row.rowId));
...
export async function getLegacyAttributeViewRowIds(databaseId: string): Promise<string[]> {
    const av = await loadAttributeViewReadOnly(databaseId);
    if (!av) throw new Error(`无法读取旧数据库：${databaseId}`);
