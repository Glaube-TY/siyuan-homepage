...
<div
    ...
    class:hp-initial-load-overlay--section={mode === "section"}
...
>
    <div class="hp-initial-load-overlay__panel">
        <div class="hp-initial-load-overlay__copy">
            <strong>{failed ? "主页加载失败" : mode === "theme" ? "正在切换主题" : mode === "section" ? "正在加载分区" : "正在准备主页"}</strong>
            <span>
                {failed
                    ? (detail ? `${detail}（已有组件和布局不会被修改）` : "请关闭当前主页标签后重新打开，已有组件和布局不会被修改。")
                    : mode === "theme"
                        ? `正在首次准备“${themeName || "主页主题"}”，已有组件与布局将保持不变…`
                        : mode === "section"
                            ? `正在准备“${sectionName || "分区"}”的组件…`
                            : detail || "正在恢复主题与组件布局…"}
            </span>
        </div>
        ...
        {#if !failed}
            <div
                ...
                role="progressbar"
                aria-label={mode === "theme" ? "主题切换进度" : mode === "section" ? "分区加载进度" : "主页加载进度"}
