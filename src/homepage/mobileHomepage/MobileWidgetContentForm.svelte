<script lang="ts">
  ...
  const countdownStyleChoice = $derived(
    form.countdownDisplaySystem === "center"
      ? "center"
      : ["list1", "list2", "card1", "card2"].includes(form.countdownStyle)
        ? form.countdownStyle
        : "list1",
  );
  ...
  function createDefaultForm(type: string): FormState {
    const defaults: Record<string, FormState> = {
      ...
      countdown: {
        ...
        countdownView: normalizeCountdownWidgetView(undefined),
  ...
  async function initialize(): Promise<void> {
    ...
    const matchingData = getMatchingData(loadedConfig);
    form = normalizeLoadedForm(widgetType, matchingData);
    if (widgetType === "countdown") {
      form.countdownDisplaySystem =
      ...
      if (plugin?.ADVANCED) {
        try {
          ...
          console.warn("[mobile content form] 纪念日筛选选项读取失败", error);
        }
      }
    }
    isReady = true;
  ...
  function toggleCountdownViewArray(key: string, value: string): void {
    const current = Array.isArray(form.countdownView?.[key])
      ? form.countdownView[key]
      : [];
    form.countdownView[key] = current.includes(value)
      ? current.filter((item: string) => item !== value)
      : [...current, value];
  }

  function selectCountdownStyle(value: string): void {
    if (value === "center") {
      form.countdownDisplaySystem = "center";
      return;
    }
    if (!["list1", "list2", "card1", "card2"].includes(value)) return;
    form.countdownDisplaySystem = "classic";
    form.countdownStyle = value;
  }
    ...
    switch (widgetType) {
          ...
          data: [
            withExistingData({
              ...
              latestDocsSortBy: normalizeRecentDocsSortBy(
                form.latestDocsSortBy,
              ),
              ensureOpenDocs:
                normalizeRecentDocsSortBy(form.latestDocsSortBy) === "openAt",
      ...
      case "TaskMan":
        return {
          ...
          data: withExistingData({
            ...
            showCompletedTasks: normalizeBoolean(form.showCompletedTasks, true),
      ...
      case "countdown":
        return {
          ...
          data: withExistingData({
            ...
            countdownCard1LocalBg: normalizeString(form.countdownCard1LocalBg),
      ...
      case "timedate":
        return {
          ...
          data: withExistingData({
            ...
            morningImageType: normalizeString(form.morningImageType, "remote"),
            ...
            simple1ShowSecond: normalizeBoolean(form.simple1ShowSecond, true),
            ...
            dial1ShowSecond: normalizeBoolean(form.dial1ShowSecond, true),
            dial1ShowMarkers: normalizeBoolean(form.dial1ShowMarkers, true),
            dial1ShowDate: normalizeBoolean(form.dial1ShowDate, true),
            dial2ShowSecond: normalizeBoolean(form.dial2ShowSecond, true),
            dial2ShowMarkers: normalizeBoolean(form.dial2ShowMarkers, true),
            dial2ShowDate: normalizeBoolean(form.dial2ShowDate, true),
            dial3ShowSecond: normalizeBoolean(form.dial3ShowSecond, true),
            dial4ShowSecond: normalizeBoolean(form.dial4ShowSecond, true),
            dial5ShowSecond: normalizeBoolean(form.dial5ShowSecond, true),
            dial6ShowSecond: normalizeBoolean(form.dial6ShowSecond, true),
            dial7ShowSecond: normalizeBoolean(form.dial7ShowSecond, true),
            dial8ShowSecond: normalizeBoolean(form.dial8ShowSecond, true),
            dial9ShowSecond: normalizeBoolean(form.dial9ShowSecond, true),
          }),
        };
      case "focus":
        return {
      ...
      case "quick-notes":
        return {
          ...base,
          data: withExistingData({
            quickNotesTitle: normalizeString(form.quickNotesTitle, "快速笔记"),
      ...
      case "childDocs":
        return {
          ...base,
          data: withExistingData({
            childDocsTitle: normalizeString(form.childDocsTitle, "📄子文档"),
      ...
      case "PicCaro":
        return {
          ...
          data: withExistingData({
            ...
            PicPaginationDyBu: normalizeBoolean(form.PicPaginationDyBu, false),
            PicPaginationPrOp: normalizeBoolean(form.PicPaginationPrOp, false),
            PicEffect: normalizeString(form.PicEffect, "slide"),
            PicSlidesPerView: normalizeString(form.PicSlidesPerView, "1"),
            PicRandomSwitch: normalizeBoolean(form.PicRandomSwitch, false),
          }),
        };
      case "CYBMOK":
        return {
      ...
      case "countdownTimer":
        return {
          ...base,
          data: withExistingData({
            advancedEnabled: normalizeBoolean(form.advancedEnabled, false),
      ...
      case "fixedAssets":
        return {
          ...
          data: withExistingData({
            ...
            fixedAssetsListLimit: normalizeNumber(form.fixedAssetsListLimit, 6),
      ...
      case "accounting":
        return {
          ...base,
          data: withExistingData({
            accountingTitle: normalizeString(form.accountingTitle, "记账"),
      ...
      case "reviewDocs":
        return {
          ...
          data: withExistingData({
            ...
            reviewDocsFutureDays: normalizeNumber(form.reviewDocsFutureDays, 7),
            reviewDocsShowDocs: normalizeBoolean(form.reviewDocsShowDocs, true),
            reviewDocsShowBlocks: normalizeBoolean(
              form.reviewDocsShowBlocks,
              true,
            ),
            reviewDocsShowNote: normalizeBoolean(form.reviewDocsShowNote, true),
            reviewDocsShowPath: normalizeBoolean(form.reviewDocsShowPath, true),
            reviewDocsShowStats: normalizeBoolean(
              form.reviewDocsShowStats,
              true,
            ),
            reviewDocsSortBy: normalizeString(form.reviewDocsSortBy, "dueAsc"),
      ...
      case "conditionDocs":
        return {
          ...
          data: withExistingData({
            ...
            conditionDocsKeyWord: normalizeString(form.conditionDocsKeyWord),
      ...
      case "latest-docs":
        return [
          ...
          limitField("limit", "显示条数", 20),
          {
            ...
            type: "select",
            label: "最近文档类型",
            options: RECENT_DOCS_SORT_OPTIONS.map((item) =>
      ...
      case "reviewDocs":
        return [
          titleField("reviewDocsTitle", "📚复习文档"),
          {
            ...
            type: "info",
            label:
              "复习计划使用本地索引，操作日志保存在插件本地并由所有复习组件共享。",
          },
        ...
        return [
          titleField("statisticalCardTitle", "统计卡片"),
          {
            ...
            options: [
              ...
              option("headingBlocksCount", "标题块数量"),
              option("paragraphBlocksCount", "段落块数量"),
              option("dueTodayTasksCount", "今日截止任务"),
              option("overdueTasksCount", "已逾期任务"),
              option("highPriorityTasksCount", "高优先级任务"),
              option("unscheduledTasksCount", "未排期任务"),
...
>
  ...
  {:else}
    <div class="mobile-content-form-body">
        ...
        <section class="mobile-form-section">
          ...
          <label class="mobile-form-field"
            ><span>组件样式</span><select
              value={countdownStyleChoice}
              onchange={(event) =>
                selectCountdownStyle(event.currentTarget.value)}
              ><option value="list1">列表 1</option><option value="list2"
                >列表 2</option
              ><option value="card1">卡片 1</option><option value="card2"
                >卡片 2</option
              ><option value="center">纪念日中心</option
              ></select
            ></label
          >
          {#if form.countdownDisplaySystem === "classic"}
            {#if form.countdownStyle === "card1"}<label
                class="mobile-form-field"
                ><span>卡片背景</span><select
                  bind:value={form.countdownCard1BgSelect}
                  ><option value="remote">远程图片</option><option value="local"
                    >本地图片</option
                  ></select
                ></label
              ><label class="mobile-form-field"
                ><span>远程背景地址</span><input
                  type="text"
                  bind:value={form.countdownCard1RemoteBg}
                /></label
              >{:else if form.countdownStyle === "card2"}<label
                class="mobile-form-field"
                ><span>卡片颜色</span><input
                  type="color"
                  bind:value={form.countdownCard2BgColor}
                /></label
              >{:else if form.countdownStyle === "list2"}<label
                class="mobile-form-field"
                ><span>列表颜色</span><input
                  type="color"
                  bind:value={form.countdownList2BgColor}
                /></label
              >{/if}
          {:else}<label
              class="mobile-form-field"
              ><span>纪念日中心布局</span><select
                bind:value={form.countdownView.viewMode}
                ><option value="list">列表</option><option value="compact"
                  >紧凑</option
                ><option value="cards">卡片</option><option value="timeline"
                  >时间线</option
                ></select
              ></label
            >{/if}
          <label class="mobile-form-field"
            ><span>数据范围</span><select
              bind:value={form.countdownView.scopeMode}
              ><option value="all">全部事件</option><option value="filter"
                >按筛选</option
              ><option value="selected">指定事件</option></select
            ></label
          >
          {#if form.countdownView.scopeMode === "filter"}<div
              class="mobile-countdown-filter-chips"
            >
              <strong>分类</strong>
              <div>
                {#each countdownCategories as category}<button
                    type="button"
                    class:active={form.countdownView.categoryIds.includes(
                      category.id,
                    )}
                    aria-pressed={form.countdownView.categoryIds.includes(
                      category.id,
                    )}
                    onclick={() =>
                      toggleCountdownViewArray("categoryIds", category.id)}
                    >{category.name}</button
                  >{/each}
              </div>
              <strong>标签</strong>
              <div>
                {#each countdownTags as tag}<button
                    type="button"
                    class:active={form.countdownView.tags.includes(tag)}
                    aria-pressed={form.countdownView.tags.includes(tag)}
                    onclick={() => toggleCountdownViewArray("tags", tag)}
                    >#{tag}</button
                  >{/each}
              </div>
            </div>{:else if form.countdownView.scopeMode === "selected"}<div
              class="mobile-countdown-filter-chips"
            >
              <strong>指定事件</strong>
              <div>
                {#each countdownCenterEvents as event}<button
                    type="button"
                    class:active={form.countdownView.eventIds.includes(
                      event.id,
                    )}
                    aria-pressed={form.countdownView.eventIds.includes(
                      event.id,
                    )}
                    onclick={() =>
                      toggleCountdownViewArray("eventIds", event.id)}
                    >{event.name}</button
                  >{/each}
              </div>
            </div>{/if}
          <label class="mobile-form-field"
            ><span>时间范围</span><select
              bind:value={form.countdownView.dateRange}
              ><option value="all">全部</option><option value="today"
                >今天</option
              ><option value="next7">未来 7 天</option><option value="next30"
                >未来 30 天</option
              ><option value="next90">未来 90 天</option><option
                value="thisMonth">本月</option
              ><option value="thisYear">本年</option></select
            ></label
          >
          <label class="mobile-form-field"
            ><span>排序</span><select bind:value={form.countdownView.sortBy}
              ><option value="nearest">最近</option><option value="priority"
                >优先级</option
              ><option value="manual">手动</option><option value="name"
                >名称</option
              ></select
            ></label
          >
          <label class="mobile-form-field"
            ><span>最大数量</span><input
              type="number"
              min="1"
              max="100"
              bind:value={form.countdownView.maxItems}
            /></label
          >
          {#if form.countdownDisplaySystem === "center"}<label class="mobile-form-field"
            ><span>跟随全局显示</span><span class="mobile-switch"
              ><input
                type="checkbox"
                checked={form.countdownView.displayMode !== "custom"}
                onchange={(event) => {
                  if (event.currentTarget.checked)
                    form.countdownView.displayMode = "inherit";
                  else {
                    form.countdownView.displayMode = "custom";
                    form.countdownView.displayOverrides = {
                      ...DEFAULT_COUNTDOWN_DISPLAY_PREFERENCES,
                      ...form.countdownView.displayOverrides,
                    };
                  }
            ...
            ></label
          >
          {#if form.countdownView.displayMode === "custom"}
            {#each [["showWeekday", "显示星期"], ["showOriginalDate", "显示原始日期"], ["showOccurrenceDate", "显示发生日期"], ["showLunarDate", "显示农历"], ["showCategory", "显示分类"], ["showTags", "显示标签"], ["showPriority", "显示优先级"], ["showCountLabel", "显示周年/年龄"], ["showNotePreview", "显示备注"], ["showLinkedNoteAction", "显示关联笔记按钮"]] as item}
              <label class="mobile-form-field"
                ><span>{item[1]}</span><span class="mobile-switch"
                  ><input
                    type="checkbox"
                    checked={form.countdownView.displayOverrides[item[0]]}
                    onchange={(event) =>
                      (form.countdownView.displayOverrides[item[0]] =
            ...
            <label class="mobile-form-field"
              ><span>日期格式</span><select
                bind:value={form.countdownView.displayOverrides.dateFormat}
                ><option value="localized">中文年月日</option><option
                  value="ymd">YYYY-MM-DD</option
      ...
      {#if widgetType === "reviewDocs" || widgetType === "focus"}
        <section class="mobile-form-section">
          <div class="mobile-form-section-title">
            <strong
              >{widgetType === "reviewDocs" ? "复习通知" : "番茄钟通知"}</strong
            >
    ...
    <div class="mobile-form-actions">
      <button type="button" class="mobile-form-cancel" onclick={onClose}
        >取消</button
      >
...
<style>
  .mobile-countdown-filter-chips {
    display: grid;
    gap: 8px;
  }
  .mobile-countdown-filter-chips > div {
    display: flex;
    flex-wrap: wrap;
    gap: 7px;
  }
  .mobile-countdown-filter-chips button {
    min-height: 40px;
    padding: 0 12px;
  ...
  .mobile-countdown-filter-chips button.active {
