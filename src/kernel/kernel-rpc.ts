 ...
 */
export async function registerRobotKernelRpc(
  host: RobotKernelHost,
  runtime: RobotKernelRuntime,
  options: RobotKernelRpcOptions = {},
): Promise<void> {
  ...
  const rpc = (
    ...
    bindings.push(
      host.registerRpc(method, async (payload) => {
        try {
  ...
  rpc("robot.getStatus", async () => ({
    ...
    providers: ["wechat", "feishu", "qq"].map((id) =>
      runtime.getProviderStatus(id as "wechat" | "feishu" | "qq"),
    ),
    runtimeDevice: runtime.getRuntimeDevice(),
    runtimeOwner: runtime.getSettings().runtimeOwner,
  }));

  rpc("robot.getSettings", () => ({
    ok: true,
  ...
  rpc("robot.saveSettings", async (payload) => {
    const settings =
      payload && typeof payload === "object"
        ? (payload as Parameters<RobotKernelRuntime["saveSettings"]>[0])
        : undefined;
  ...
  rpc("robot.validateProviderCredentials", async (payload) => {
    ...
    return {
      ok: true,
      configured: await runtime.validateElectronProviderCredentials(provider),
    };
  });
  ...
  rpc("robot.getHistory", async (payload) => {
    ...
    return {
      ok: true,
      items: await runtime.getHistory(Math.max(1, Math.min(500, limit))),
    };
  });
  ...
  rpc("robot.sendAutomationMessage", async (payload) => {
    ...
    return runtime.sendAutomationMessage({
      provider,
  ...
  rpc("robot.renameSession", async (payload) => {
    ...
    if (!conversationId || !title.trim())
      return { ok: false, errorCode: "invalid_session_title" };
  ...
  rpc("robot.startPairing", async (payload) => {
    ...
    const ttlMs =
      payload &&
      typeof payload === "object" &&
      typeof (payload as Record<string, unknown>).ttlMs === "number"
  ...
  rpc("robot.getPairing", async () => ({
    ok: true,
    pairing: await runtime.getPairing(),
  }));
  ...
  rpc("robot.ingestExternalMessage", (payload) => {
    ...
    void runtime
      .ingestExternalMessage(payload as NormalizedRobotMessage)
      .catch((error) => {
        host.log.error({
  ...
  rpc("robot.registerElectronProvider", async (payload) => {
    ...
    if (provider !== "feishu" && provider !== "qq")
      return { ok: false, errorCode: "invalid_provider" };
    const status =
      p.status && typeof p.status === "object"
        ? (p.status as Record<string, unknown>)
        : {};
    const statusValue =
      typeof status.status === "string" &&
      ROBOT_PROVIDER_STATUSES.has(status.status)
        ? (status.status as RobotProviderStatus)
        : "offline";
    const availability =
      typeof status.availability === "string" &&
      ROBOT_PROVIDER_AVAILABILITIES.has(status.availability)
    ...
    const account =
      ...
      typeof (status.account as Record<string, unknown>).accountId === "string"
        ? {
            accountId: String(
              (status.account as Record<string, unknown>).accountId,
            ),
            authenticated: true,
            ...(typeof (status.account as Record<string, unknown>)
              .displayName === "string"
              ? {
                  displayName: String(
                    (status.account as Record<string, unknown>).displayName,
                  ),
                }
              : {}),
          }
  ...
  rpc("robot.confirm", async (payload) => {
    const confirmationId =
      payload &&
      typeof payload === "object" &&
      typeof (payload as Record<string, unknown>).confirmationId === "string"
        ? ((payload as Record<string, unknown>).confirmationId as string)
        : "";
    if (!confirmationId)
      return { ok: false, errorCode: "invalid_confirmation_id" };
  ...
  rpc(
    "robot.wechat.startLogin",
    () =>
      options.wechat?.startLogin?.() ?? {
        ok: false,
        errorCode: "not_implemented",
      },
  );
  rpc(
    "robot.wechat.getLoginState",
    () =>
      options.wechat?.getLoginState?.() ?? {
        ok: false,
        errorCode: "not_implemented",
      },
  );
  rpc(
    "robot.wechat.submitVerifyCode",
    (payload) =>
      options.wechat?.submitVerifyCode?.(payload) ?? {
        ok: false,
        errorCode: "not_implemented",
      },
  );
