...
- 保存每日 `totals` 和每个文档的 `doc contribution` 到 `heatmap-daily-index.json`，支持内容块、字数、创建文档数和更新文档数。
...
- 创建文档数按根文档 `created` 日期计数，每篇文档每天最多贡献 1；更新文档数按文档内任意块的更新日期去重统计，并在每篇文档 contribution 中保存已观察到的更新日期集合，文档再次更新时新增当天记录，不抹掉之前已观察到的更新日。
- 旧版热力图索引升级文档计数时，复用索引内已有文档 ID，通过分批 `id IN (...) AND type='d'` 精确查询补齐根文档时间，不执行全库扫描。
