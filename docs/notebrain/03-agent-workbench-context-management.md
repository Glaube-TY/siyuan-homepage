...
- `currentTurn`：当前用户问题、当前 scope 轻量信息、当前 attachedDocs 轻量信息，以及 `runtimeNow`（当前运行时间）。
- `runtimeNow`：每轮 conversationContext 构建时生成的当前本地时间，包含 ISO 字符串、本地可读时间、时区、时区偏移分钟和毫秒时间戳。用于帮助 Planner 理解“今天、昨天、本周、现在、刚才、最近”等相对时间；它不是聊天消息，不进入历史压缩，不属于工具 observation，不改变 Planner 决策边界。
...
stageSummary 不预设固定格式，也不限定具体领域。它是当前会话内的通用历史阶段摘要，只要求忠实概括最近未总结对话中对后续继续对话有帮助的信息。适合输出的情况包括：连续多轮对话后形成了值得后续保留的关键信息或结论；当前回答包含对后续对话有参考价值的信息、结论、约束或上下文线索；用户明确要求保存阶段摘要；或不总结会导致后续压缩丢失重要上下文。简单寒暄、确认、过渡回复、无新增信息时不要输出。
...
- **硬阈值（forceCompressionRatio，默认 0.9）下的特殊处理**：
  - 发送前如果 usageRatio >= forceCompressionRatio，普通压缩后会重新估算上下文用量。
  - 如果普通压缩后仍 >= forceCompressionRatio，且还有未被最新 stageSummary 覆盖的 completed turns，会触发一次 Emergency Context Compaction（调用 LLM 生成 emergency stageSummary）。
  - Emergency 成功后，会立即按新 stageSummary 边界 retry 普通压缩，然后再次重新估算。
  - 如果最终仍 >= forceCompressionRatio，则阻断本轮发送，不向 Planner 继续 runAgentTurn；只通过 toast/notice 提示用户，不写聊天消息，不带刷新/重试按钮。
  - 低于 force 阈值时，保留现有 no_op 后继续策略。
  - 常规压缩不调用 LLM；Emergency Compaction 只在硬阈值兜底时触发一次，不递归。
...
可信 grounding 来源包括：
1. `read_docs` 成功返回的非空正文。
2. `attached_doc_hydration` 成功返回的非空正文。
3. `web_read_page` 成功读取的非空网页正文。
4. 历史 references 中只有 `grounded:true` 的元信息可进入 `conversationContext`。
5. 用户本轮 attachedDocs。
6. 当前 scope 中明确的 docId/rootDocId/docIds。

搜索候选和结构结果只能作为线索，不能自动成为 footer reference。只有 content-level 证据才能进入最终引用。
...
- 常规阶段摘要压缩不调用 LLM；仅在达到硬阈值（forceCompressionRatio，默认 90%）且常规压缩无法安全压缩时，允许触发一次性的 Emergency Context Compaction 由 LLM 生成应急阶段摘要。
- Emergency Compaction 不是普通工具，不写 ObservationLog，不读工具 observation，不写长期记忆，只生成当前会话内阶段摘要。
- 不做 Reasonix archive。
- 不做长期记忆。
- 不写回思源文档。
- 不新增业务工具。
