...
最终回答的显示流式是 UI 层逐步展示已经生成的最终回答，不改变 Planner / Tool 边界，也不实现真实模型 token streaming。

## 最终回答生成

当前实现采用 Planner + Composer 两阶段：
- **Planner** 输出 answer 草稿（含 references、可选 stageSummary），决定工具调用和回答时机。
- **Composer（final-answer-composer）** 接收 Planner 草稿和当前轮正文证据，生成完整、连贯、可直接呈现给用户的最终回答正文。

Composer 不选工具、不改工具调用、不做业务决策，只负责把草稿和证据组织成用户可读回答。如果当前轮没有可靠正文证据，Composer 会要求说明缺少可靠来源，不允许基于候选线索写确定性结论。
...
4. 当前 conversationContext 里已有的历史 references — **只有 `grounded:true` 的历史 reference 才可信**。
5. 用户本轮 attachedDocs。
6. 当前 scope 中明确的 docId/rootDocId/docIds。

搜索候选和结构结果只能作为线索，不能自动成为 footer reference。未通过 grounding 的 reference 会被丢弃，不进入 footerReferences / citedReferences / agentMemory / conversationContext。丢弃时会写 `REFERENCE_DROPPED_UNGROUNDED` debug event，但默认不 console。
