...
interface SkillContract {
  ...
  roleInstruction: string;     // 角色指令：AI 在启用本 Skill 后的身份与默认目标
  whenUseful: string;          // 适合场景（仅 prompt 说明，不得被代码用来自动启用 / 自动路由）
  boundary: string;            // 边界 / 禁止事项
  toolNames: readonly string[];// 声明关联工具集合，不代表自动调用或优先顺序
...
- MCP tools
- skill-provided tools（由 Skill 自带、注册到 ToolRegistry 的工具）
...
interface ToolContract {
  ...
  boundSkillName?: string;     // 由哪个 Skill 引入（不写 boundSkill）
...
const builtinKnowledgeBaseQa: SkillContract = {
  ...
  guidance: "可用工具能力：知识图谱/文档树、搜索、范围文档列表、文档子树上下文、候选读取、历史引用读取、回答。工具可以组合使用，没有强制顺序。是否组合、如何组合由你根据问题和 observation 自己决定。证据足够时由你自己选择 answer。",
  priority: 100,
  enabledByDefault: true,
  buildPromptSection(ctx) { /* 渲染 Skill prompt 段落 */ },
  getTools(ctx) { /* 返回该 Skill 声明关联的 ToolManifest 子集 */ },
};
```
...
- `skills/system/answer`：系统 answer tool 已落地，提供最终回答能力。
...
- `get_doc_tree_context`：查看文档子树上下文
- `read_candidate_docs`：读取候选文档
...
**禁止出现位置**：`getPlannerToolManifest` 渲染结果、`SkillContract.getTools` 返回的 manifest、`PlannerRuntimeContext.toolManifest`。命中后必须删除或迁移到白名单路径。
...
- `answerDraft` **不**含真实 docId / blockId / path；只含 `evidenceMode` / `body` /
  `displayedReferenceHandles`（安全句柄）。
- 真实引用映射由 `EvidencePack` 层处理（`registerAnchor` / `resolveHandle`）。
...
- `skills/builtin/kb-retrieval/adapters/` 适配底层 readers，注入 scope 和 handle mapping 保存器。
...
- toolNames 只是声明关联工具集合，不代表自动调用或优先顺序。
...
- `list_knowledge_map` returns neutral notebook handles, safe node handles, sanitized titles, hierarchy, and counts only.
- `get_conversation_used_references` returns safe reference handles, sanitized titles, and safe counts only.
- Neither tool reads document body content.
- Neither tool decides the next business action.
- Neither tool calls search, read, or answer automatically.
